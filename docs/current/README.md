...
| server-go | Core API, auth, storage, realtime Hub, BPP routing | Does not run plugin/remote-agent processes | HTTP, browser WS, plugin WS, remote WS |
...
| 3. Module architecture | Read the owner view for each major subsystem | [Server](server/), [client](client/), [admin](admin/), [plugin](plugin/) | [remote-agent](remote-agent/) |
...
| 6. Security and permission boundaries | Review auth, admin rails, and the remote filesystem boundary | [Security](security/) | [API/auth/admin rails](server/api-auth-admin-rails.md), [admin server rail](admin/server-rail.md), [remote filesystem boundary](remote-agent/filesystem-boundary.md) |
...
| 9. Implementation Anchors | Jump from architecture areas to stable code ownership anchors | Implementation Anchors sections in each document | [server](server/), [plugin](plugin/), [remote-agent](remote-agent/) |
| 10. Known Gaps / Architecture Debt | Track current mismatches and where detailed debt is recorded | [Known gaps](known-gaps.md) | Module-local gap notes in [admin](admin/), [remote-agent](remote-agent/), and [E2E](e2e/) |
...
- Remote agent daemon: `packages/borgee` (single binary)
