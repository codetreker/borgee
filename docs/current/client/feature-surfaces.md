...
This is not the admin SPA. It is a user rail surface backed by user endpoints and shared app state, so it can be visible in the normal shell without granting admin session capabilities. The channel-management tab renders action availability, exposes server-owned agent require-mention policy controls, and explains `@Everyone` behavior. Delete/archive availability requires both channel ownership and the matching current permission state; leave availability excludes creator-owned channels. The Settings tab itself does not execute leave, delete, archive, owner-transfer, notification, collapse, sort, pin, group, or private-indicator controls.
