...
- 反约束: `data-ai-only` / `data-agent-only` / `data-visibility-scope` / `data-agent-visible-only` 0 hit (蓝图 §185 透明协作设计)
...
- X2 错码字面沿用 — CM-5 不自起 `cm5.x2_conflict` / `agent_collision` / `artifact.x2_conflict` / `x2_lock_held` 同义词, 强制复用 CV-4 #380 ⑦ 既有错码 `artifact.locked_by_another_iteration` byte-identical (cm5stance.TestCM51_X2ConflictLiteralReuse server-side 反约束守同源)
...
| `CM5_FORBIDDEN_VISIBILITY_DOM_ATTRS` | 反约束 (server cm5stance.TestCM51_NoBypassTable) | 设计 ⑤ 反 ai_only |
