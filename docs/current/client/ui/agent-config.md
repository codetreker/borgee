...
> 蓝图: `agent-lifecycle.md §2.1` (用户完全自主决定 agent 的 name/prompt/能力/model) + `plugin-protocol.md §1.4` (Borgee owns the documented config fields) + §1.5 (热更新分级 — server PATCH 后经 BPP frame `agent_config_update` 推送给 plugin 端 reload, 走 AL-2b #481 wire; client 端不订阅 ws — 走 PATCH/GET 同步)
...
owner 在 agent settings 下编辑本人 agent 的 7 个配置字段: name / avatar / prompt / model / capabilities / enabled / memory_ref。Save 提交 PATCH `/api/v1/agents/{id}/config`，server schema_version 严格递增 (server-stamped monotonic UPSERT)。
...
`onMount + Save 后 re-fetch` is the acceptance §4.1.d evidence that the agent reload path uses polling: the client uses GET and does not subscribe to WS push frames (蓝图 §1.5 BPP `agent_config_update` 留 AL-2b)。
...
- 反向约束 不订阅 push frame
- Negative constraint: toast synonym drift 0 hit
- grep 检查 (gh#701 drift fix): packages/ + docs/qa/ 全树 `data-form="agent-config"` 字面 0 hit (容器是 section, 不是 form)

## 9. Layout checks (gh#698)

6 label inline `style="display: block"` values prevent form label/input overlap on mobile viewport widths. Do not replace them with `.form-group` / `.form-field` classes because those classes do not exist in this project. checkbox label 例外用 `style="display: flex; align-items: center; gap: 8px"` 让 ☐ 跟 "启用" 同行.
