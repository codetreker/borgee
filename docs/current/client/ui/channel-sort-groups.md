...
Private channel visibility is currently a leading `.channel-hash.channel-private-indicator` slot with `data-private-indicator="true"`. The slot keeps its base glyph (`.channel-hash-base` holds `#` for non-archived rows and `📦` for archived rows) and renders an absolutely-positioned `svg.channel-lock-badge` (`data-private-lock="true"`, `aria-hidden="true"`) in the slot's upper-right corner for any private channel — including archived private rows, which therefore show `📦` plus the lock overlay. Active (non-archived) private rows still carry `data-private="true"`. Public rows render `#` with no lock overlay; archived public rows render `📦` with no lock overlay. Pinned rows can add `data-pinned="true"` and `.channel-pinned-indicator` in the trailing accessory area. Public preview rows can add `.channel-item-preview` and `.preview-badge`; private non-member rows should not be assumed visible from this sketch because server ACL remains authoritative. Reverse anchor: the literal CJK character `锁` MUST NOT appear in any sidebar row — private identity is icon-on-icon, not character-replacement.
