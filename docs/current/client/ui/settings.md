...
> **Path separation**（ADM-0 constraint）：用户 SPA `components/Settings/SettingsPage.tsx` 跟 admin SPA `admin/pages/SettingsPage.tsx` 同名共存不混用 — admin SPA 走 `react-router` + `/admin-api/*` cookie；用户 SPA 走 App-level state + `/api/v1/*` cookie。The cookie and endpoint namespaces are separate, which prevents cross-surface requests（REG-ADM0-001/002 共享底线）。

## 2. SettingsPage 骨架

`SettingsPage.tsx`：

- header：`<` 返回按钮（`data-page="settings"` + `[data-action="open-settings"]` 反查出处）+ "设置" 标题。
...
`PrivacyPromise.tsx` — 用户隐私承诺页，**默认展开不可折叠**（R3 requirement; do not wrap it in `<details>`）。
...
- acceptance：`docs/qa/acceptance-templates/adm-1.md` (11 验收项，9/11 ✅ + 2/11 ⏸️ ADM-2 后续)
- registry：`docs/qa/regression-registry.md` REG-ADM1-001..006 (6 🟢)

## 7. ADM-2 扩展 (Phase 4 第二个 admin-model milestone)
...
该组件通过 `lib/api.ts::getMyImpersonateGrant / createMyImpersonateGrant / revokeMyImpersonateGrant` 读取、创建、撤销业主 24h impersonate 授权。设计 ⑦ + content-lock §3。
...
`AdminActionsList` 通过 `lib/api.ts::getMyAdminActions` 拉取数据。设计 ④：user 只见自己。限制：不渲染 raw `actor_id` UUID（server-side sanitizer 会省略，client 也不读取）。
...
刷新策略（限制：不通过 WebSocket frame 刷新；设计 ⑥ 与 CHN-4 一样采用轮询，不新增 ws subscription）：
...
1. `<PrivacyPromise/>` (ADM-1 隐私承诺锁定)
2. `<ImpersonateGrantSection/>` (ADM-2 业主授权)
3. `<AdminActionsList/>` (ADM-2 影响记录)

注释字面: `// ADM-2.2 业主授权 24h impersonate (acceptance §4.2.a; 设计 ⑦ + content-lock §3)` + `// ADM-2.2 影响记录 (acceptance §4.1.c; 设计 ④ 只见自己 + content-lock §4 字面)`

### 7.5 lib/api.ts 扩展 (4 helpers)

`getMyAdminActions` / `getMyImpersonateGrant` / `createMyImpersonateGrant` / `revokeMyImpersonateGrant` 与 `getMyLayout / putMyLayout` (CHN-3.2) 使用同类 REST helper 结构：调用 `request<T>()` helper，并复用 `BASE`（空字符串，vite proxy 同源）。限制：不引入新 ws subscription，与设计 ⑥ 一致，保持轮询刷新。
...
- 蓝图: `docs/blueprint/current/admin-model.md` §1.4 (谁能看到什么 + three constraints) + §3 (impersonation_grants 数据模型片段) + §4.1 R3 (ADM-1 文案兑现出处)
- spec: `docs/implementation/modules/adm-2-spec.md` §2-3
- content lock: `docs/qa/adm-2-content-lock.md` §1+§2+§3+§4
- 原则: `docs/qa/adm-2-stance-checklist.md` (7 条原则 + 10 反向约束)
- acceptance: `docs/qa/acceptance-templates/adm-2.md` §4.1.c+§4.2.a (9/11 ✅ + 2/11 ⏸️ 后续)
