...
## 1. capability label 单一来源 — `lib/capabilities.ts`
...
DOM data-attr 单一来源 (跟 content-lock §2 byte-identical):
...
- ❌ RBAC 角色名脱节 (英 admin/editor/viewer/owner + 中 管理员/编辑者/查看者) 0 hit
- ❌ inline 字面散落 (capabilityLabel 单一来源 production 仅 1 hit)
- ❌ admin god-mode UI 永久独立 (capabilityLabel 不挂 components/admin/* 路径)
- ❌ thought-process 5-pattern + typing-indicator 脱节 (跟 RT-3 #616 一致)

## 4. bundle 单一来源 — `lib/capability-bundles.ts` + `components/BundleSelector.tsx`
