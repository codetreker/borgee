...
| `push` | 解析 `e.data.json()` payload → 渲染通知 (`self.registration.showNotification`)；payload shape 与 `internal/push/mention_notifier.go` 保持一致；`mention` kind 渲 `${from} mentioned you in #${channel}` + body；`agent_task` kind 渲 busy/idle 状态，busy 必带 subject 蓝图 §1.1 ⭐；未知 kind drop silently |
