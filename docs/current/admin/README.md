...
**约束**: server diff ≤13 行 production (D4 sanitizer +5 + D6 gate +5 + struct field +3); 0 endpoint URL / 0 schema migration / 0 routes.go 改; admin-rail 单一来源 (CookieName `borgee_admin_session` / loginRequest / handleMe writeJSON) 字面 byte-identical 不动. ADM-0 §1.3 admin/user 路径分叉红线守.
...
- 3 option value byte-identical 跟 server `admin_endpoints.go::handleAdminAuditLog ?archived=` enum 字面单一来源 (不一致 = 改两处 client + server)
