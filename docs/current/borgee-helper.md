...
| `packages/borgee-helper/install/borgee-helper.service` | systemd unit (Linux dedicated OS user `borgee`) |
...
| `grants` | `internal/grants/grants.go` (94) + `sqlite_consumer.go` (110) + 2 `_test.go` | Read-only SQLite consumer for HB-3 `host_grants`, using the single SELECT WHERE `revoked_at IS NULL AND (expires_at IS NULL OR expires_at > now)` |
...
| `reasons` | `internal/reasons/reasons.go` (27) + `_test.go` (51) | Denial reason enum (`grant_revoked` / `grant_expired` / `path_outside_whitelist` 等) shared with locked UI copy |
...
- **常驻无 sudo** (蓝图 §1.3) — The daemon runs as the dedicated OS user/group `borgee`; install/`borgee-helper.service` keeps the literal `User=borgee Group=borgee`.
