...
> Design source: [`adm-3-spec.md`](../../implementation/modules/adm-3-spec.md) v1 §0 ① DL-1+DL-2 + ADM-2/ADM-3 #586 audit_events unchanged + ② no schema changes + four-source enum source + ③ no user-rail endpoint / admin path isolation

## 1. 文件清单

| 文件 | 行 | 角色 |
|---|---|---|
| `internal/api/admin_audit_query.go` | 260 | four-source enum source + AdminAuditMultiSourceHandler + MultiSourceAuditQuery UNION ALL + queryAuditEvents + queryAgentEvents + sortByTSDesc |
| `internal/api/admin_audit_query_test.go` | 250 | 9 server unit (4 source literal match + 4 source UNION + filter + InvalidSource + TimeRange + InvalidTimeRange + OrderTSDesc + UserCookieRejected + UnauthRejected + LimitClamp + HostBridgePlaceholder) |
| `internal/server/server.go` 扩 | +5 | NewAdminAuditMultiSourceHandler.RegisterAdminRoutes(s.mux, adminMw) wired |
| `internal/api/agent_log_filter_test.go` 改 | +1 allow | AL-8 reverse-grep for `/admin-api/v1/audit/<not-log>` adds `/multi-source` as the only allowed exception (spec §0 ② authorized endpoint) |
...
| `packages/client/src/__tests__/MultiSourceAuditPage.test.tsx` | 130 | 7 vitest (AUDIT_SOURCES literal match + DOM 出处 + per-source row + SOURCE_LABEL + empty state + error alert + filter triggers fetch + synonym-denial case) |
...
`AuditSources` slice ordering is the source for the four entries; any change must update the server const, client `AUDIT_SOURCES`, and i18n `SOURCE_LABEL` together.
...
| 字面 | baseline | 当前 | 反查 |
|---|---|---|---|
| ADM-2 既有 /admin-api/v1/audit-log | unchanged | unchanged ✅ | 0 改 |
| audit_events 表 schema | ADM-3 #586 RENAME | unchanged ✅ | 0 ALTER, 0 column add |
| DL-2 channel_events/global_events | DL-2 #615 | unchanged ✅ | 仅 SELECT 消费 |
| user-rail has no audit/multi-source route | n/a | 0 hit ✅ | ADM-0 §1.3 isolation rule |
| admin path is isolated | unchanged | unchanged ✅ | uses adminMw with the admin cookie path |
...
- reasons.IsValid #496 / NAMING-1 #614 / DL-2 mustPersistKinds enum source pattern
...
- AL-8 既有 reverse-grep 测试加 `/multi-source` 唯一允许例外
...
- 4 source const check: `grep AuditSource{Server,Plugin,HostBridge,Agent} admin_audit_query.go` ==4 hit
...
- AUDIT_SOURCES cross-layer match: server const + client AUDIT_SOURCES + SOURCE_LABEL 三处保持一致
