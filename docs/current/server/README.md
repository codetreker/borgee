...
`POST /api/v1/channels/{channelId}/artifacts` 创建 artifact（CV-3.2: kind ∈ {markdown, code, image_link}, code 必含 metadata.language ∈ 11 项白名单, image_link 必含 metadata.kind ∈ ('image','link') + URL https only）。**CHN-2.1 设计 ② DM 反约束 (PR #407)**: `ch.Type == "dm"` → 403 `{code: "dm.workspace_not_supported", message: "DM 无 workspace, 跟 channel 拆"}` (蓝图 §1.2 字面禁; grep 检查 `dm.workspace_not_supported` count≥1 锁 handler)。

`GET/POST /api/v1/artifacts/{id}/commits`、`POST /api/v1/artifacts/{id}/rollback` — CV-1.2 commit/rollback。Commit 单源接受 `?iteration_id=<id>` query: 同 atomic UPDATE 把对应 `artifact_iterations.state` running → completed 并写 `created_artifact_version_id` (CV-4.2 设计 ②, 反约束: 不开 `POST /iterations/:id/commit` 旁路)。
...
设计反查: 入参字段反约束断言 reject `hidden` / `muted` / `pinned` / `group_id` (设计 ②); server 不算 MIN-1.0 pin 算法 (client 端事, 设计 ③ + ⑥); 不发 `LayoutChangedFrame` push (设计 ⑥ — grep 检查 `WSEnvelope.*position|push.*frame.*position|fanout.*user_channel_layout` count==0)。失败 toast 文案锁 "侧栏顺序保存失败, 请重试" 跟 client #371 / 文案锁 ④ 三源同步。
...
**AL-3.1/AL-3.2 presence_sessions**：`internal/presence/` 提供 `SessionsTracker`（DB-backed read/write）+ `PresenceWriter` 接口；hub 的 register/unregister 钩到 `TrackOnline`/`TrackOffline`，写 `presence_sessions` 表（migration v=12）。`PresenceWriter` 接口让 unit test 可注 fake，单 binary 模式不需 DB-backed presence 时也能 no-op。多端 last-wins：同 user 多 session 各占一行，最后一条 offline 才标 user 整体下线。读端给 mention 路由 / sidebar / DM-2.2 fallback 共用，避免 AL-3 设计 ③ 反约束的 `IsOnline`/`IsAgentOnline` 重复实现漂移。
