...
> 蓝图出处: data-layer.md §5 阈值哨 (db_size / wal_pending / write_lock / row_count)
> 设计沿用: [`dl-3-spec.md`](../../implementation/modules/dl-3-spec.md) §0 ① DL-1+DL-2 byte-identical + ② 0 schema 改 + 4 阈值哨 单一来源 + auto cold archive + ③ 0 endpoint 改 + admin god-mode 永不挂
...
| `internal/server/server.go` 扩 | +6 | NewThresholdMonitor(...).Start(s.ctx) wire (sweeper 同精神沿用) |
...
`DefaultThresholds()` 单一来源, 反 inline 字面脱节.
