# RT-3 ⭐ multi-device fanout + 活物感 4 态 + thinking subject 反向约束 (≤80 行)
...
> 蓝图出处: [`realtime.md`](../../blueprint/current/realtime.md) §0 + §1.1 (thinking subject ⭐) + §1.4 (活物感 4 态)
> 设计沿用: [`rt-3-spec.md`](../../implementation/modules/rt-3-spec.md) §0 ① DL-1+RT-1 byte-identical + ② 4 态 enum 单一来源 + thinking subject 必带 + ③ 0 schema/endpoint 改

## 1. PresenceState 4 态 enum 单一来源 (`internal/datalayer/presence.go`)
...
**反向约束**: 4 态封闭枚举 (反 PresenceStateTyping/Composing/Idle/Pending/Loading 漂 — grep 检查 0 hit).

## 2. thinking subject 反向约束 (蓝图 §1.1 ⭐ 关键纪律)

`internal/bpp/task_lifecycle.go` 加 `ThinkingErrCodeSubjectRequired = "thinking.subject_required"` wire-level reason code 单一来源. server 走 `ValidateTaskStarted` 单一来源 (errSubjectEmpty sentinel) reject 空 subject; client 防御层 `markRT3Presence` 在 thinking 态 + 空 subject 时 drop, 反"假 loading" 漂.
...
| `components/RT3PresenceDot.tsx` (54 行) | 4 态 UI + DOM data-attr 单一来源 + tooltip 字面 byte-identical |
| `__tests__/RT3PresenceDot.test.tsx` (9 case PASS) | 4 态 + last-seen + thinking 反向约束 + multi-device 后写覆盖前 |
...
- ❌ typing-indicator 真启 (永久不挂, 跟 thought-process 5-pattern 守护链一致)
