...
> 落地: PR feat/refactor-2 · R2.1 (4 helper 单一来源) + R2.2 (caller 跟随) + R2.3 (脱节 #11 部分 + #12 收口) + closure
> 蓝图出处: refactor 元 milestone (跟 REFACTOR-1 / INFRA-3 / INFRA-4 同等级)
> 设计沿用: [`refactor-2-spec.md`](../../implementation/modules/refactor-2-spec.md) v1 §0 ① 行为不变量 + ② 4 helper 单一来源 + ③ 0 schema/endpoint
>
...
|---|---|---|---|
| `Unauthorized` | ≥100 | helper 内 1 (其他 100 处 caller 走 helper) ✅ | mustUser 单一来源承载, status 401 byte-identical |
| `Invalid JSON` | ≥5 | helper 内 1 (其他 5 处 canonical-shape 走 helper) ✅ | decodeJSON 单一来源承载 |
| `Agent not found` | ≥10 | helper 内 1 (其他 8 处 path-id 走 helper) ✅ | loadAgentByPath 单一来源承载 |
| `layout.dm_not_grouped` | ≥19 (REFACTOR-1) | ≥19 ✅ | RejectDM 组单一来源 (channel_helpers.go::requireChannelMember, REFACTOR-1 #611 已立) |
| `dm.edit_only_in_dm` | 7 | 7 ✅ | RequireDM 组单一来源 (dm_4_message_edit.go), byte-identical 不动 (audit 反转: 不归一字面) |
...
- REFACTOR-1 #611 4 helper 单一来源 (mustUser/decodeJSON/loadAgentByPath/fanoutChannelStateMessage 续作 + RejectDM 组字面单一来源继承)
- BPP-3 #489 PluginFrameDispatcher / reasons.IsValid #496 / TEST-FIX-3 #610 fixture 单一来源 (helper 单一来源同模式)
...
- ❌ helper-3 DM-gate 三错码归一 **撤** — dm_4 `dm.edit_only_in_dm` (DM-only 403) vs chn_6/7/8/layout `layout.dm_not_grouped` (RejectDM 400) 同字段反向条件不同 status 不同 reason, 字面归一破 user-facing 错码契约. 真值: 双向各自字面 byte-identical 单一来源, 设计目标已达成.
- ❌ helper-4 ACL 双重 → 单一来源 **撤** — 双层 `IsChannelMember && CanAccessChannel` 是 security correctness 设计, 不是脱节. 实测折叠破 TestAP5_*PostRemovalReject. 真值: write 路径双层 AND / list 路径单 CAC — 已分清.
