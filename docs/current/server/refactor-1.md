...
> 蓝图出处: refactor 元 milestone (跟 INFRA-3/INFRA-4 同等级)
> 设计沿用: [`refactor-1-spec.md`](../../implementation/modules/refactor-1-spec.md) §0 ① 行为不变量 + ② 4 helper shared path + ③ 0 schema/endpoint

## 1. 4 helper 单一来源 (`internal/api/`)

| Helper | 文件 | 收口处 | 收口效果 |
|---|---|---|---|
| `requireChannelMember(w, r, store, channelID, opts)` | `channel_helpers.go` (89 行) | chn_6 / chn_7 / chn_8 / chn_15 / layout 5 处 | 4-step preamble (auth → load → DM gate → member/creator) shared path; ChannelACLOpts{RejectDM, RequireCreator} 携 5 处变体 |
| `parseMessageEditHistory(raw)` | `message_edit_history.go` (34 行) | dm_7 + cv_15 2 处 | 11 行 verbatim duplicate 合一; 旧 parseEditHistoryEntries / parseCommentEditHistory 删 |
...
| `handlePinToggle(w, r, pin)` | `chn_6_pin.go` 内 | chn_6 pin/unpin | thin wrapper 模式跟 chn_7 handleMuteToggle / chn_15 handleReadonlyToggle 对齐 (脱节收口) |

## 2. caller 列表锁定 (9 文件 touched)
...
| `dm.edit_only_in_dm` | 7 | 7 ✅ | literal 不动 |
...
- BPP-3 #489 PluginFrameDispatcher / reasons.IsValid #496 / TEST-FIX-3 #610 fixture shared-source pattern (helper 模式沿用)
- chn_7 #523 / chn_15 既立 toggle 模式 (chn_6 对齐脱节收口)
- content-lock §1 字面绑 (DM-gate 4 字面跨 helper 不脱节)
- audit-forward-only / owner-only ACL 链 (RetentionOverride helper audit literal match)
...
- ❌ #6 DM-gate 13 hits 三种错误码统一 — 留 REFACTOR-2 (需先讨论 error code source design)
- ❌ #8/#9/#10/#11 admin-list / loadAgent / cursor-push / ACL 脱节 — 留 REFACTOR-2/3
