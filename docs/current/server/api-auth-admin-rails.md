...
Helper enrollment flow: a signed-in owner creates an enrollment row, receives a one-time local enrollment secret, and a local Helper claims the row with a helper device id. The create response also returns the operator-facing `enrollment_token` (= `<enrollment_id>.<enrollment_secret>`, the form `install --token` consumes) plus a ready-to-paste `install_command` (`npx @codetreker/borgee-remote-agent install --server <wss://host> --token <token>`); both are derived per-request (scheme + host from `X-Forwarded-Proto` / `X-Forwarded-Host` with `r.TLS` / `r.Host` fallback) and shown exactly once, since the server persists only the secret's digest. Later heartbeat, rotation, and helper-originated uninstall requests present the current persistent Helper credential and matching helper device id. Revoked or uninstalled rows block future heartbeat and rotation writes.
