...
CORS emits `Access-Control-Allow-Origin` (echoing the specific request origin, never `*`) together with `Access-Control-Allow-Credentials: true` only for an allowed origin, and never the credentials flag alone. In production the allowed set is exactly the configured `CORS_ORIGIN`. In development the set adds a strict loopback allowlist — origins whose host is exactly `localhost`, `127.0.0.1`, or `::1` over http/https on any port — rather than reflecting arbitrary origins; lookalikes such as `localhost.evil.com` are rejected by exact host match. This keeps the vite dev stack and direct cross-origin loopback dev clients working while preventing a third-party page from reading credentialed responses cross-origin even if the service is mistakenly started in development mode on a reachable host.
...
The per-IP key used by the `auth` and `anon` buckets is derived in a trusted-proxy-aware way so a client cannot rotate buckets by forging headers. By default (`TRUSTED_PROXY_COUNT=0`) the key is the connection's `RemoteAddr` host only, and the client-controlled `X-Forwarded-For` and `X-Real-IP` headers are ignored entirely; trusting them would let an unauthenticated attacker mint a fresh `auth:<ip>` bucket per request and bypass login brute-force throttling. When the service runs behind one or more real reverse proxies, operators set `TRUSTED_PROXY_COUNT=N` in the environment so the rightmost `N` hops of `X-Forwarded-For ++ RemoteAddr` are treated as proxies and the real client just left of that trusted window is used; an attacker's left-injected entries fall outside the window and are ignored, and a chain shorter than `N` safely degrades to the leftmost (never an arbitrary value). Leaving `TRUSTED_PROXY_COUNT` unset behind a proxy fails closed — all proxied clients collapse into one bucket and over-throttle rather than under-throttle.
