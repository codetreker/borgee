...
Helper enrollment state is stored in `helper_enrollments`. The row is the server-side Helper identity/status authority and binds `owner_user_id`, `org_id`, host label, optional `helper_device_id`, closed allowed-category JSON, status, last-seen timestamps, active credential lifecycle metadata, terminal revoke/uninstall timestamps, and the latest update-detection snapshot (`updates_available_json` + `last_update_check_at`, both nullable). One-time enrollment secrets and persistent Helper credentials are stored only as digests; raw values are returned only once by the API path that creates, claims, or rotates them. The active credential digest is replaced on rotation, while `credential_created_at`, `credential_rotated_at`, and `credential_generation` record lifecycle metadata. The update-detection snapshot is a latest-wins idempotent projection — Helper POSTs its installed-plugin versions on each tick, the server compares to the current signed manifest, and stores the per-class drift list as JSON for the enrollment serializer to surface. The table is separate from `remote_nodes`, `host_grants`, and `user_permissions`.
