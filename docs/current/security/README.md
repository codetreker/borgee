...
helper outbound prereq: Helper service config -> exact public HTTPS origin allowlist + Helper-owned state roots -> no poll request yet
...
| Helper outbound prerequisite | Helper daemon startup config | exact Borgee public HTTPS origin allowlist plus platform service/sandbox path boundaries; localhost/private/link-local/metadata origins are rejected by default even over HTTPS | systemd/launchd/sandbox assets and daemon startup validation; local UDS remains the only inbound listener | local Helper-owned queue/status/audit-handoff state roots only; no job payloads, raw credentials, private content, full environment dumps, or Remote Agent data | prerequisite config is Helper-rail only and does not reuse Remote Agent credentials, reverse WebSocket transport, host grants, file-proxy status, or permission fallbacks |
...
- Helper enrollment currently provides identity/status authority: claim, heartbeat, credential rotation, revoke, and helper-originated uninstall state. Helper jobs currently provide user-rail typed enqueue authority only. Helper outbound service prerequisites provide exact public-origin startup validation that rejects localhost/private/link-local/metadata origins by default, narrow AF_UNIX/IPv4/IPv6 or macOS remote-TCP sandbox shape, and explicit Helper-owned state roots only. They do not provide Helper polling, raw command execution, service lifecycle execution, local policy execution, bounded logs, result settlement, or Configure OpenClaw success state.
