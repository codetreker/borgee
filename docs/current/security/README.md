...
Resource ownership gates appear alongside capability checks. Remote nodes, Helper enrollments, Helper jobs, host grants, runtime owner actions, and user privacy audit views all use owner scoping to keep cross-user access from being implied by broad credentials. Helper job enqueue also binds org id, enrollment state, Helper freshness, allowed category, job type, target agent/channel access for channel-bound requests, server-owned agent config or plugin binding state, and server-owned OpenClaw/plugin manifest binding; host label alone is display metadata, not authority. Helper local job policy repeats owner, org, enrollment id, Helper device id, credential generation, status, category, revocation, stale credential, and expiry checks before any future action can proceed.
...
- Helper enrollment currently provides identity/status authority: claim, heartbeat, credential rotation, revoke, and helper-originated uninstall state. Helper jobs provide user-rail typed enqueue plus Helper-credential poll/lease/ack/result settlement with deterministic non-success reasons, redacted failure messages, and opaque audit/log references. OpenClaw install/config, Borgee plugin channel binding, and service lifecycle job records are closed typed jobs with server-derived payloads and server-owned manifest/path/artifact/domain/service-ID bindings; they still do not execute OpenClaw, write plugin config locally, or call service managers. Helper outbound service prerequisites provide exact public-origin startup validation that rejects localhost/private/link-local/metadata literal origins by default, narrow AF_UNIX/IPv4/IPv6 or macOS remote-TCP sandbox shape, explicit Helper-owned state roots only, and bounded non-root Helper boot/crash restart settings. Helper local job policy exists as a pure pre-action evaluator for delivered job views, signed runtime manifests, artifact bytes, allowlists, service IDs, and sandbox/profile affordances. These pieces do not resolve allowed hostnames, inspect DNS answers or CNAME chains, provide raw command execution, service-manager execution, policy-to-action execution, raw/bulk log upload, or Configure OpenClaw success state.
