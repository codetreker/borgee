...
helper outbound prereq: Helper service config -> exact HTTPS origin allowlist + Helper-owned state roots -> no poll request yet
...
| Helper outbound prerequisite | Helper daemon startup config | exact Borgee HTTPS origin allowlist plus platform service/sandbox path boundaries | systemd/launchd/sandbox assets and daemon startup validation; local UDS remains the only inbound listener | local Helper-owned queue/status/audit-handoff state roots only; no job payloads, raw credentials, private content, full environment dumps, or Remote Agent data | prerequisite config is Helper-rail only and does not reuse Remote Agent credentials, reverse WebSocket transport, host grants, file-proxy status, or permission fallbacks |
