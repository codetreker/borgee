...
helper jobs:   user credential -> owner/org/enrollment/category/job-type checks -> queued typed metadata only
...
| Helper job enqueue | user cookie or Bearer API key | authenticated owner/org plus Helper enrollment, allowed category, closed job type, fresh last seen, typed payload, server config binding, and active-window idempotency | user-authenticated `POST /api/v1/helper/enrollments/{enrollmentId}/jobs`; no Helper poll/lease/result route | job serializer exposes safe metadata and digests only; no owner/org internals, credentials, command text, paths, domains, payload body, or logs | enqueue records typed queued intent only; it is not raw command, service lifecycle, or Configure OpenClaw success authority |
...
Resource ownership gates appear alongside capability checks. Remote nodes, Helper enrollments, Helper jobs, host grants, runtime owner actions, and user privacy audit views all use owner scoping to keep cross-user access from being implied by broad credentials. Helper job enqueue also binds org id, enrollment state, Helper freshness, allowed category, job type, and server-owned agent config state; host label alone is display metadata, not authority.
