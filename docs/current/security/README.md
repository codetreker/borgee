...
- Helper enrollment currently provides identity/status authority: claim, heartbeat, credential rotation, revoke, and helper-originated uninstall state. Helper jobs currently provide user-rail typed enqueue authority only. Helper outbound service prerequisites provide exact public-origin startup validation that rejects localhost/private/link-local/metadata literal origins by default, narrow AF_UNIX/IPv4/IPv6 or macOS remote-TCP sandbox shape, and explicit Helper-owned state roots only. They do not resolve allowed hostnames, inspect DNS answers or CNAME chains, provide Helper polling, raw command execution, service lifecycle execution, local policy execution, bounded logs, result settlement, or Configure OpenClaw success state.
