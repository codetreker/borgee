...
- gh#718 — 全 repo 黑话整治 (P2, in-flight). sed 时代 12 PR (#721/#723/#725/#726/#727/#728/#729/#730/#731/#732/#733/#734) 已合, 用户拍方法学违反 issue 原文重做; v13+ 按语境读真做, 已合 37 PR / 207 文件 / 余 ~333 文件 / 估 ~55 PR. v17-v49 累计 0/1334 = 0% 漏率 33 PR (含 v21 + v34 + v38 三次 conditional-then-fixed). **3 大里程碑达成** (docs/current/client/ 33 .md 文件 + docs/current/server/ ~32 文件 / 5 子目录 + packages/borgee-helper/ 24 Go 文件 production+test+e2e). Layer 7 防注意力疲劳 26 次成功. v34/v38 教训触发 Layer 7 词典扩 (single-source-of-truth + stance + SSOT + 拆死 + 二闸 + 单源 + 锁精准式). v45v3 → v46 → v47 → v48 → v49 真合规 PR #1-#5 (反字典化 / 真 per-context / 诚实同译 / iterator var 不动 / 包名不动 / content-lock 字面不动 / anchor check / 保守 sweeping — 标杆五件套). sed era 平反 (v49 修 7a/7b/7c 漏率 82% 名单中 3/4 文件). 无独立 tasks/ 目录.
