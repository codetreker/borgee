...
- Use existing channel authority if sufficient; if the required channel authority is missing, this task must wait for or explicitly depend on Phase 2 channel authority work rather than bypassing ACL checks.
...
Acceptance slice:
- A reviewer can verify channel binding rechecks channel ACL/authority, cannot bind through owner/org/enrollment alone, and records any dependency on Phase 2 channel authority before implementation proceeds.

Parallelism:
- Can run after task 1. May run alongside service lifecycle task if shared Configure OpenClaw files do not conflict.
