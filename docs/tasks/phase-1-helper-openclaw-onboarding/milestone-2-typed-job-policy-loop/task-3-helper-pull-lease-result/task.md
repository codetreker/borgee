# task-3-helper-pull-lease-result

Purpose:
- Make enrolled Helper retrieve and settle server-authorized jobs through outbound-only transport.

Scope:
- Add Helper outbound poll/long-poll, lease, ack, result upload, retry/backoff, idempotency, cancellation, and stale credential handling using the service/sandbox prerequisite from task 2.
...
Parallelism:
- Blocks task 5. Can run alongside task 4 after task 2 if Helper transport and local policy files are separable.
