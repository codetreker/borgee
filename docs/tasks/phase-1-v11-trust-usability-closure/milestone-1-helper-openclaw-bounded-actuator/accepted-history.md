# Accepted History: Helper / OpenClaw Bounded Actuator

This file preserves the accepted Helper/OpenClaw task history after the coarse v1.1 replan. The former finer-grained Helper/OpenClaw phase directory has been deleted; accepted task docs and progress now live directly under this canonical milestone.

| PR | Commit | Accepted scope | Canonical task home |
|---|---|---|---|
| #934 | `547f869` | Helper enrollment/status foundation | `task-1-helper-enrollment-model-and-status` |
| #936 | `1ca5f95` | Helper credential rotation/revoke lifecycle | `task-2-helper-credential-rotation-and-revoke` |
| #937 | `2872905` | Helper status UI and current-doc sync | `task-3-helper-status-ui-and-current-sync` |
| #938 | `64d56f1` | Helper job envelope and enqueue authority | `task-4-job-envelope-and-enqueue-authority` |
| #939 | `96dc0dc` | Helper outbound service prerequisites | `task-5-helper-outbound-service-prereq` |
| #942 | `642fb57` | Local policy, manifest, and sandbox profile | `task-7-local-policy-manifest-and-sandbox-profile` |
| #943 | `c2c61e6` | Helper pull, lease, ack, and result transport | `task-6-helper-pull-lease-result` |
| #954 | `419c5bf` | Bounded status logs and revoke settlement | `task-8-bounded-status-logs-and-revoke-settlement` |
| #956 | `5575b53` | OpenClaw install and agent config jobs | `task-9-openclaw-install-and-agent-config-jobs` |
| #958 | `ad50575` | Borgee plugin channel binding job | `task-10-borgee-plugin-channel-binding-job` |
| #963 | `d8d179e` | Service lifecycle boot/crash reliability | `task-11-service-lifecycle-boot-crash` |
| #964 | `3450d8c` | Configure OpenClaw terminal UI | `task-12-configure-openclaw-terminal-ui` |

`task-12-configure-openclaw-terminal-ui` closed this milestone through PR #964 (`3450d8c`).
