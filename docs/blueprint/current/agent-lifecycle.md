...
- 用户**完全自主**决定 agent 的：
  - 名字
  - prompt（在哪个 runtime 平台配置）
  - 能力（哪些 tool / MCP）
  - LLM 模型选择
- Borgee 端只有：`agent 行 + plugin connection 配置`
- 创建 UI：填名字 → 选 runtime（v1 只有 OpenClaw）→ 配置 plugin → 完成
...
Borgee 引导用户安装 remote-agent（v1 只 Mac/Linux）
    ↓
remote-agent 帮用户在本机完成 OpenClaw 设置
    ↓
...
已经在用 OpenClaw / Hermes 的种子用户可以**直接配置 plugin**接入，跳过 remote-agent。这是早期口碑放大的关键路径，不能为了 onboarding 去掉。
...
| Onboarding | 单条路径（添加 agent → 自动配置） | 增加 plugin marketplace UI |
...
| **故障（error）** | runtime 报错或失联 | API key 失效、超限、网络断、进程崩溃 | Phase 2 |

> **2026-04-28 4 人 review #5 决议**: busy / idle 在 Phase 2 不实现 (source 必须是 plugin 上行的 `task_started` / `task_finished` frame; 没有 BPP 时只能做临时占位实现；若占位实现进入 v1，后续仍需删除，属于一次性实现)。**Phase 2 在线列表只承诺 online / offline + error（可解释故障态）**, G2.4 验收范围对应收窄。busy/idle 跟 BPP 同期 (Phase 4 AL-1) 落地。
> ⚠️ **§11 文案要求 (G2.4 必须满足的验收条件)**: Phase 2 Sidebar 显示 online / offline 两态时, 不准用"灰点 + 不说原因"这种模糊呈现, 必须明确文案 ("已离线" 而不是模糊的 idle 灰点)。
...
> 这是 owner 维护 AI 团队的核心 UX——agent 像同事一样会发生故障，owner 要能看到原因并直接处理。
...
| 行为 | 入口 | 含义 |
|------|------|------|
| **B 禁用**（默认） | 主路径，agent 设置一键 | 停止接收消息，所有产出 / 对话保留可查；可一键恢复 |
...
| remote-agent 作为 runtime 管家 | ❌ 当前只是文件代理 | **重写**——见下文 §4 风险 |
| 四态 + 故障可解释 | 仅有 online / offline | 状态模型扩展 + 故障原因码 + 修复入口 UI |
| Owner 自助禁用入口 | admin 有 disable cascade，owner 端无 UI | 增加 owner 端禁用按钮 |
...
| token 失效后果 | 最多导致文件读取失败 | **攻击者可远程在用户机器上执行任意进程** |
