...
> **2026-04-28 4 人 review #5 决议**: busy / idle 在 Phase 2 不实现 (source 必须是 plugin 上行的 `task_started` / `task_finished` frame; 没有 BPP 时只能做 stub, 若 stub 进入 v1 后续仍需删除，属于一次性实现)。**Phase 2 在线列表只承诺 online / offline + error 旁路**, G2.4 签字范围对应收窄。busy/idle 跟 BPP 同期 (Phase 4 AL-1) 落地。
> ⚠️ **§11 文案要求 (G2.4 硬条件)**: Phase 2 Sidebar 显示 online / offline 两态时, 不准用"灰点 + 不说原因"这种模糊呈现, 必须明确文案 ("已离线" 而不是模糊 idle 灰)。
