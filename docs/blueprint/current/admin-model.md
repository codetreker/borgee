...
- Admin **不是** org 成员（[concept-model §2](concept-model.md) 已确定）——不是 user，不属于任何 org。
- 物理隔离的运维 SPA + 独立鉴权域（独立 cookie / 独立路由）。
- 这条把现状从"历史兼容问题"升级成"有意设计"——**身份模型边界清晰**：
  - 产品代码不为 admin 写分支（admin 没有 agent、没有 channel、不在 user 表的 role 里进入产品侧流程）
  - 否则会引出"admin 有没有自己的 agent / org / DM"等一系列身份边界问题
...
| 阶段 | 入口 |
|------|------|
| **首个 admin** | 环境变量 `ADMIN_USER` / `ADMIN_PASSWORD` 自举创建 | 防止空表自举漏洞, 唯一启动入口 |
| **后续 admin** | 现有 admin 在 SPA 里**新建 admins 行** (输入用户名 + 密码, 不是把任何 user 提升为 admin) | 写入 audit log |
...
这条边界跟 [host-bridge §1.3](host-bridge.md) 的"装时轻、用时问、问时有理由"采用同一规则——Borgee 默认信任用户的隐私。

> **ADM-3 #570 规则扩展 (audit 事件仅向前追加，覆盖所有 actor type)**: audit 单表 (`audit_events`, ADM-3.1 v=43 RENAME 自 `admin_actions`) 存放所有 actor type — admin / system / plugin lifecycle / 未来类型, forward-only **不可 DELETE/UPDATE** (清理任务只设置 `archived_at`); admin 仍使用 `/admin-api/*` 单独路径 (平台运维专用 endpoint). 别名 view `admin_actions` 保持向后兼容到 Phase 5+ 废弃阶段。
...
> ❌ 撤销: ~~`admin_grants` 表~~ — admin 不再由 user 提升产生, 不需此表。
...
> 文案目的: 让用户在第一次进设置页就能读懂"admin 跟我有什么关系", 不需要看文档 / 不需要问客服。这条是 §0 "强权但不窥视" 原则对用户的兑现。
