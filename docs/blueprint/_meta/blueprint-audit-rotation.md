...
> 飞马 · 2026-04-28 · 防止 R3 重排后文档与代码再次脱节
...
|---|---|---|---|
| **每周一 (固定)** | 飞马 | PROGRESS.md vs `gh pr list --state merged --search "merged:>=$(date -d '7 days ago' +%F)"` | 对比已合并的 PR 编号是否出现在 PROGRESS.md 记录中 |
| **每 milestone ✅** | 该 milestone 负责人 (战马/野马) | docs/current/server/{migrations.md, data-model.md, http-api.md} | `grep -n 'v=[0-9]' registry.go` 对照 migrations.md §7 行数; `git diff main -- internal/store/models.go` 对照 data-model.md |
| **每 Phase 退出关卡** | 飞马 + 烈马 | 全量核对三层文档 (blueprint § ↔ implementation/modules/*.md ↔ docs/current/) | 记录到 `docs/implementation/00-foundation/phase-N-N+1-transition-audit.md` (#212 模板) |
| **每 PR merge 前** | 评审人 (飞马) | PR body `## Current 同步` 段是否填 (规则 6 lint) | CI lint 已检查, 评审人复核非 N/A 时查看 docs/current/ diff |
...
**触发**: 任何修改 cookie 名 / migration v / handler 函数名 / 中间件名 / 路由前缀 / 文件名引用 的 PR merge 后, 下次 Phase 关卡审计 **必须运行**。
...
**判定**: docs/current 含旧常量任意一处 → 审计这一行标 🔴 P0, 由飞马提交修复 PR 后才放行 Phase 退出。

## 3. 红线

❌ Phase 退出无审计文档 · ❌ PROGRESS.md 记录漏掉一周 · ❌ PR 改 schema 但 docs/current diff 为空 · ❌ milestone PR merge 后负责人不补 docs/current
