...
Resume from: resolve `HB-RA-1` lock blockers or split its already-safe product guardrails into a smaller locked anchor before Phase planning.
...
|---|---|---|---|---|---|---|---|
| `HB-RA-1` | Helper bounded remote actuator for Configure OpenClaw | OPEN | PENDING | gh#681, gh#659 | `remote-actuator-design.md`; `migration-analysis.md` §2 | - | Resolve sandbox, credential, manifest, revoke, and Helper-vs-Remote-Agent blockers. |
...
| `CT-1` | Client truthfulness and forbidden-state visibility | OPEN | PENDING | gh#724 | `migration-analysis.md` §5 | - | Decide forbidden-state shape and whether e2e reverse proof is a blueprint invariant. |
| `PS-1` | Privacy scope guard | OPEN | PENDING | gh#654 | `migration-analysis.md` §6 | - | Treat as lock candidate after Security confirms no existing admin/privacy/server-side boundary is removed. |
...
Default version judgment is minor continuation. The only major-trigger cluster is `HB-RA-1`: if the current helper sandbox/isolation model cannot support declared, schema-bound, pre-authorized host-management jobs, the trust pillar must be rewritten before lock. Removing helper isolation, adding a host command channel, or making Borgee the runtime owner is a major decision, not a minor continuation.

## Lock Candidates And Open Blockers

### `HB-RA-1` Helper bounded remote actuator

Safe product guardrails that can be locked once Architecture and Security confirm wording:
...
- Sandbox profile: exact macOS/Linux write paths, network domains, service permissions, outbound polling permission, and the current Linux AF_UNIX-only service restriction.
- Revoke race rules: safe action boundaries and terminal status precedence for running jobs.
- Helper vs Remote Agent boundary: user-facing IA may move, but credentials, grants, and enforcement rails must stay separate.

`docs/tasks/681-remote-agent-openclaw/` is a legacy intake folder. It must not be treated as an execution path until this anchor is locked and a Phase/Milestone path exists.
...
- Decide whether e2e reverse proof becomes a blueprint invariant or only gh#724 task acceptance.

### `PS-1` Privacy scope guard

Lock candidate:

- gh#654 means no new user-facing privacy/compliance product expansion. It does not remove backend security, admin, impersonation, capability, audit, data minimization, or rail-separation boundaries.

Security blocker:

- Reconcile any cleanup language with the current admin privacy contract. User-visible security controls that already exist must not be removed under the label of avoiding compliance-product scope.
...
Run another Blueprintflow lock review after the blockers above are resolved or split. If at least one row becomes `LOCKED / PENDING`, route to `bf-phase-plan` to create the `docs/tasks/` Phase/Milestone plan and first task seed. Do not run milestone breakdown or task execution before that lock.
