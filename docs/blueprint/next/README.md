...
Resume from: route locked `HB-RA-1A` guardrails to Phase planning, carry locked `PS-1` as a privacy-scope reverse check, and continue `HB-RA-1B` execution-contract discussion.
...
|---|---|---|---|---|---|---|---|
| `HB-RA-1A` | Helper bounded actuator product guardrails | LOCKED | PENDING | gh#681, gh#659 | `remote-actuator-design.md` §1.1-§1.2; `migration-analysis.md` §2.1 | - | Route only the locked bounded-helper guardrails to Phase planning. Do not inherit `HB-RA-1B` execution-contract blockers by whole-doc reference. |
...
Route `HB-RA-1A` to `bf-phase-plan` for guardrail-only Phase/Milestone planning. Keep `PS-1` as a locked reverse-check guardrail for affected work. Keep `HB-RA-1B` and the other `OPEN / PENDING` rows in lock review until their blockers are resolved or split. Do not run milestone breakdown or task execution before the relevant anchor lock.
