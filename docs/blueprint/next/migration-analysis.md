...
### §2.1 HB-RA-1A locked guardrail impact

`HB-RA-1A` locks the product boundary for bounded Helper actuator work only: explicit local enrollment first, Web-side Configure OpenClaw may enqueue bounded typed jobs after enrollment, helper transport is outbound poll / long-poll, server enqueue and helper local policy both validate authority and revocation, long-lived services stay non-sudo, status/logs are bounded and redacted, and Helper rails stay separate from Remote Agent rails.
...
- Sandbox and Linux outbound poll: exact macOS/Linux write paths, allowed network domains, outbound polling permission, and the current Linux AF_UNIX-only long-lived service restriction.
...
### §2.6 QA acceptance / negative checks
...
### §6.1 gh#654 precise privacy scope guard
