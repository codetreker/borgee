...
// (user OR agent — 约定 ⑥ 同表同语义) when the target is online; offline
// targets go through the owner system DM fallback path (DM-2.2
// mention_dispatch.go), which does NOT use this frame.
//
// Blueprint依据: docs/blueprint/current/concept-model.md §4 (agent 代表自己 —
...
//      AnchorCommentAdded 共一根 sequence (RT-1 spec §1.1, 不允许:
...
// MentionPushedFrame, schema 收口在此一个地方.
package ws
...
// 80 字符按 #362 spec §0 设计 ② 钉住; 防 raw body 完整泄露给 mention target
...
// 不一致 between caller and frame would re-introduce raw body leakage.
...
// BroadcastToUser (target-only fanout — 约束: 不抄送 owner; offline
