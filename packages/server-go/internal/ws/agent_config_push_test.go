...
//
// 出处: docs/qa/acceptance-templates/al-2b.md §2.1 (delivery latency
// hard-line ≤1s + cursor 共序) + §2.2 (幂等 reload — 同 idempotency_key
// 重发 plugin 端 dedup, server stateless 反向约束).
package ws_test
...
// allocated (蓝图 §1.5 反向约束 "runtime 不缓存": frame dropped, plugin
...
// 共一根 sequence (反向约束: 不另起 plugin-only 通道).
...
// §2.1 原则 ① cursor 共序 — AL-2b push 跟 RT-1.1 PushArtifactUpdated
// 共一根 sequence (跟 anchor_comment_frame_test / iteration_state_changed_
// frame_test 同模式 — 反向约束: 不另起 channel).
func TestAL_PushAgentConfigUpdate_SharedSequenceWithRT1(t *testing.T) {
	...
	if c2 <= c1 {
		t.Errorf("AL-2b cursor must be strictly above prior RT-1 cursor; c1=%d c2=%d (反向约束 共一根 sequence)", c1, c2)
	}
...
// (filled + zero-tail 双 snapshot — 反向约束 不挂 omitempty).
func TestAL_PushAgentConfigUpdate_FieldByteIdentity(t *testing.T) {
	...
	// Empty blob + zero schema_version — 反向约束: 7 字段全序列化.
	...
	// 反向约束: 7 keys (始终序列化, 不挂 omitempty).
	count := strings.Count(wire, ":")
	if count < 7 {
		t.Errorf("wire must serialize all 7 fields; saw %d ':' (omitempty 脱节)", count)
	}
}
...
func TestAL_PushAgentConfigUpdate_NoCursorAllocator(t *testing.T) {
	...
	// allocator; 跟 hub_test setupTestHub 一致. 此 test 仅留字面注释 +
