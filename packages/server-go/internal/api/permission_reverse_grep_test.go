...
// (走 IsValidCapability 单一来源, 设计第 3 条).
func TestAP_HandlerHelperOnly(t *testing.T) {
	...
	for _, f := range scanGoFiles(t, apiDir, false) {
		...
		if pat.Match(b) {
			t.Errorf("%s: contains auth.Capabilities[ — must use auth.IsValidCapability(name) helper (AP-4-enum 设计第 3 条)", f)
		}
	}
}
...
func TestAP_ReverseGrep_HardcodeCapability(t *testing.T) {
	...
	for _, f := range scanGoFiles(t, apiDir, false) {
		...
		if m := pat.Find(b); m != nil {
			t.Errorf("%s: hardcode capability literal %q — must use auth.* const (AP-4-enum 设计第 2 条)", f, string(m))
		}
	}
}

// TestAP_ReverseGrep_DirectMapAccess — Capabilities["..."] = ... 仅
// init() 唯一写 (设计第 1 条). 扫 auth/*.go (排除 _test).
	...
	for _, f := range scanGoFiles(t, authDir, false) {
		...
		if matches := pat.FindAll(b, -1); matches != nil {
			for _, m := range matches {
				t.Errorf("%s: direct map mutate %q (AP-4-enum 设计第 1 条 — 仅 init() 走 Capabilities[c]=true 变量, 禁字面)", f, string(m))
				hits++
			}
		}
	}
