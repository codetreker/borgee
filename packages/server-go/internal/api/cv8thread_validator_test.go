...
//   - 设计第 2 条 human reply skips validator
package api_test
...
// TestCV_HumanReplyOnComment_OK 验证设计第 2 条: human reply on an
...
// TestCV_AgentReplyThinking_Reject 验证设计第 3 条 4-pattern reject 字节级一致
	...
	for _, b := range bodies {
		t.Run(b, func(t *testing.T) {
			...
			if d["code"] != "comment.thinking_subject_required" {
				t.Errorf("errcode 字节级一致检查失败: got %v want comment.thinking_subject_required", d["code"])
			}
		})
	}

	// Cross-check: agent with concrete subject succeeds.
...
// TestCV_ReplyOnReply_Reject 验证设计第 4 条: depth 2 → 400 thread_depth_exceeded.
func TestCV_ReplyOnReply_Reject(t *testing.T) {
	...
	if d["code"] != "comment.thread_depth_exceeded" {
		t.Errorf("errcode 字节级一致检查失败: got %v want comment.thread_depth_exceeded", d["code"])
	}
}

// TestCV_ReplyOnNonComment_Reject 验证设计第 4 条: reply target 必须 artifact_comment 类型.
func TestCV_ReplyOnNonComment_Reject(t *testing.T) {
	...
	if d["code"] != "comment.reply_target_invalid" {
		t.Errorf("errcode 字节级一致检查失败: got %v want comment.reply_target_invalid", d["code"])
	}
}
