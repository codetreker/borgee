...
// ErrNoHelperEnrolled signals there is no claimed helper enrollment
// for the owner that can receive a service.lifecycle job. The
// RuntimeHandler maps this to HTTP 400 `no_helper_enrolled` so the
// UI shows an explicit precondition error instead of silently
// flipping the DB column without dispatching the job (issue #1046).
var ErrNoHelperEnrolled = errors.New("api: no helper enrolled for owner")

// RuntimeLifecycleInput is the request to dispatch a runtime
// lifecycle operation to a specific enrollment. Operation must be
// one of the values accepted by the daemon-side jobpolicy
// (start/stop/restart/reload/enable/disable); the RuntimeHandler
// only emits start + stop today.
type RuntimeLifecycleInput struct {
	OwnerUserID  string
	OrgID        string
	EnrollmentID string
	AgentID      string
...
// RuntimeLifecycleResult describes the helper job that was enqueued.
// The RuntimeHandler echoes the ID back in the API response so an
// operator looking at the runtime status DM can trace the
// corresponding helper job by ID.
type RuntimeLifecycleResult struct {
	JobID   string
	Created bool
}

// EnqueueServiceLifecycle enqueues a `service.lifecycle` job carrying
// `{target:"openclaw", operation:<op>}` for the supplied owner +
// enrollment, then best-effort pushes it to a connected daemon via
// the WS adapter (REST poll fallback covers disconnected daemons).
//
// Idempotency key is `runtime-<agentID>-<operation>`: a second rapid
// click on the same button within the 5 min TTL returns the existing
// queued job (Created=false). After the daemon terminally completes
// the previous job, the next click creates a new row.
//
// Issue #1046: replaces the silent failure where the RuntimeHandler
// flipped only the DB column without ever asking the daemon to start
// or stop the systemd unit on the helper VM.
func (h *HelperJobsHandler) EnqueueServiceLifecycle(ctx context.Context, input RuntimeLifecycleInput) (RuntimeLifecycleResult, error) {
	if h == nil || h.Repo == nil {
		return RuntimeLifecycleResult{}, errors.New("helper jobs handler not configured")
	}
	op := strings.TrimSpace(input.Operation)
	switch op {
	case "start", "stop", "restart", "reload", "enable", "disable":
	default:
		return RuntimeLifecycleResult{}, datalayer.ErrHelperJobSchemaInvalid
	}
	payload := `{"target":"openclaw","operation":"` + op + `"}`
	job, created, err := h.Repo.EnqueueForUser(ctx, datalayer.EnqueueHelperJobInput{
		...
		SchemaVersion:  1,
		PayloadJSON:    payload,
		IdempotencyKey: "runtime-" + input.AgentID + "-" + op,
	}, h.now())
	if err != nil {
		return RuntimeLifecycleResult{}, err
	}
	// Best-effort immediate WS push so the daemon sees sub-second
	// latency; failure is silently absorbed because the REST poll
	// fallback / connect-hook drain still delivers (mirrors the
	// handleEnqueue path).
	h.tryPushAfterEnqueue(ctx, input.EnrollmentID)
	return RuntimeLifecycleResult{JobID: job.ID, Created: created}, nil
}
