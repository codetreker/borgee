...
//                              one_shot/always; content-lock §1.① 双向锁定)
...
//
// 反向约束 (hb-3-spec.md §0 + §3 + 原则 §1+§2+§3):
//   - 设计 ① schema 单一来源 — HB-3 持 ownership; HB-2 daemon (Rust crate
...
//     通道延伸 + 跨四 milestone 单测守护链: HB-1 install + HB-2 host-IPC + BPP-4
//     dead-letter + HB-3 grants 改 = 改四处单测守护链).
...
//
// v0 原则: forward-only, no Down(). IF NOT EXISTS 守 idempotency. 跟
