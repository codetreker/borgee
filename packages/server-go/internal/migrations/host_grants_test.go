...
// identical 跟 hb-3-spec.md §1 BPP-3.1 + 原则 §1 设计 ① schema 单一来源.
func TestHB_CreatesHostGrantsTable(t *testing.T) {
	...
	if len(cols) != len(want) {
		t.Fatalf("column count 脱节: got %d, want %d (HB-3.1 schema 9 列)",
			len(cols), len(want))
	}
	for _, c := range cols {
		w, ok := want[c.name]
...
// TestHB_NoDomainBleed pins 原则 §2 设计 ② 字典分立 (host vs runtime),
...
func TestHB_NoDomainBleed(t *testing.T) {
	...
	forbidden := []string{
		...
		"cursor",             // RT-1 envelope cursor 真分清
		"org_id",             // org 隔离走 users.org_id 单一来源
		"source",             // BPP 单一来源跟 AL-1b 同模式
		"set_by",             // 反人工伪造
	...
	for _, f := range forbidden {
		if have[f] {
			t.Errorf("forbidden column %q present (字典分立反向约束 — host vs runtime)", f)
		}
	}
}
...
// TestHostGrants_Idempotent pins forward-only 原则 — re-running the migration
...
func TestHostGrants_Idempotent(t *testing.T) {
	...
	e.Register(hostGrants)
	if err := e.Run(0); err != nil {
		t.Errorf("re-run should be idempotent (forward-only 原则): %v", err)
	}
}
