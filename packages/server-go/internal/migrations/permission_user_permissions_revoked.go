...
// Spec brief: docs/implementation/modules/ap-2-spec.md (v0, cfa3869)
// §0 设计 ① + §1 拆段 AP-2.1.
//
...
//   - 不添加 NOT NULL — revoked_at NULL = active, 跟 AP-1 ABAC 行为零变.
//   - 不添加 default 值 — NULL 是合法终态.
//   - 不添加 FK — 跟 user.org_id / channels.org_id 同精神 (业务校验 server 层).
...
// merge order determines the version, and later work moves forward (跟 CV-2 v1 spec §2 v=14 三方撞号 sequencing 协议同).
//
