...
// gate (用于 endpoint 错误响应 body, 跟 AP-1 const 单一来源同模式).
//
...
// returns ("", false) — empty == 历史 / unset, NOT cross-org evidence.
//
// Scope format (蓝图 §1.2 三层):
//   - "*"               → no resource, ("", false) (skip org gate)
//   - "channel:<id>"    → channel.org_id from store
//   - "artifact:<id>"   → artifact.channel_id → channel.org_id (CV-1 原则
...
func resolveScopeOrgID(s scopeOrgResolver, scope string) (string, bool) {
	...
	if strings.HasPrefix(scope, "artifact:") {
		// artifact 跟 channel 同 org (CV-1 原则 ① 归属=channel + CM-3 #208).
...
// HasCapability is the single 单一来源 capability check — all endpoint
...
//     default; agent 不享, 蓝图 §1.4 字面不变, 见 IsAgent 守)
//
...
// 历史路径 (跟 AP-1 现网行为零变, 原则 ⑥).
//
// 反向约束 (spec §2 + 蓝图 §1.4 + §2 不变量 + AP-3 原则 ①③⑤⑦):
...
//   - cross-org agent path 走同 单一来源 (BPP-1 #304 org sandbox 同源).
//
...
func HasCapability(ctx context.Context, s *store.Store, permission, scope string) bool {
	...
	// AP-3 原则 ① cross-org owner-only gate — 高于 wildcard 短路, 在
