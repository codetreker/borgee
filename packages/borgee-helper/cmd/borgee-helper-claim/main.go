...
// Package main — borgee-helper-claim CLI (closes #968 reboot/crash chain end-to-end).
//
// Operator workflow:
...
// bake into the .deb / .pkg, and a single binary that the operator runs once
// keeps the install asset surface smaller.
package main
...
const (
	defaultLinuxStateDir = "/var/lib/borgee-helper"
	defaultMacStateDir   = "/Library/Application Support/Borgee/Helper"

	defaultHelperUserLinux = "borgee-helper"
	defaultHelperUserMac   = "_borgee-helper"
...
func main() {
	if err := runCLI(os.Args[1:], os.Stdout, os.Stderr); err != nil {
		fmt.Fprintln(os.Stderr, "borgee-helper-claim:", err)
		os.Exit(1)
	}
}

func runCLI(args []string, stdout, stderr io.Writer) error {
	fs := flag.NewFlagSet("borgee-helper-claim", flag.ContinueOnError)
	fs.SetOutput(stderr)
	...
	if os.Geteuid() == 0 {
		...
		if err := chownToUser(*credentialFile, *helperUser, group); err != nil {
			...
			// where the .deb postinst has not yet created the user, the
			// operator may need to re-run after install completes.
			fmt.Fprintf(stderr, "borgee-helper-claim: warn: chown %q to %s:%s failed: %v\n", *credentialFile, *helperUser, group, err)
		}
	}
