...
//   1. Operator runs `borgee-helper-claim` against the API server.
...
//   3. systemd-equivalent: we then spawn the borgee-helper daemon with
...
func buildClaimCLI(t *testing.T) string {
	t.Helper()
	tmp := t.TempDir()
	binPath := filepath.Join(tmp, "borgee-helper-claim")
	cmd := exec.Command("go", "build", "-o", binPath, "../cmd/borgee-helper-claim")
	cmd.Stdout = os.Stderr
	cmd.Stderr = os.Stderr
	if err := cmd.Run(); err != nil {
		t.Fatalf("go build claim cli: %v", err)
	}
	return binPath
}
...
func TestClaimHeartbeatE2E(t *testing.T) {
	...
	// Step 1: operator runs claim CLI.
	...
	runDir := t.TempDir()
	socketPath := filepath.Join(runDir, "borgee-helper.sock")
	...
	// roots; in production the .deb postinst creates those. For the e2e
	// test we pass --allowed-state-roots=<tmp> so we can use the tmpdir
