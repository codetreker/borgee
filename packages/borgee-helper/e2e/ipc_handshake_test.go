...
// Package e2e covers the HB-2 IPC handshake integration behavior.
//
// hb-2-v0d-e2e-spec.md §1 case-2 IPC handshake:
//   - start the daemon
//   - dial UDS
//   - send handshake `{"agent_id":"a1"}\n`, then send a Request and read a Response
//   - reject malformed handshakes by closing connections that omit agent_id
//
// Design note (hb-2-v0d-e2e-spec.md §0 design ②): the integration build tag
// limits this coverage to Linux/macOS POSIX UDS support.
package e2e
...
func startDaemon(t *testing.T) (socket string, cleanup func()) {
	...
	if _, err := os.Stat(socketPath); err != nil {
		_ = cmd.Process.Kill()
		_ = cmd.Wait()
		// landlock_restrict_self EPERM means the runner lacks NoNewPrivs/CAP_SYS_ADMIN.
...
// TestHB2DE_IPCHandshake_RoundTrip starts the daemon, connects over UDS, sends
// a complete handshake plus a list_files request, and reads a response.
func TestHB2DE_IPCHandshake_RoundTrip(t *testing.T) {
	...
	// Note: ACL checks will likely reject this because the seeded scope and probe
	// path differ; the assertion is protocol-level and only requires a daemon response.
	...
	// status in {ok, rejected} proves that handshake and dispatch worked; "failed"
	// indicates a protocol bug instead of a silent abort.
...
// TestHB2DE_IPCHandshake_RejectsMalformed verifies that a handshake without
// agent_id closes the connection instead of accepting an unauthenticated stream.
func TestHB2DE_IPCHandshake_RejectsMalformed(t *testing.T) {
	...
	// Send a handshake with missing agent_id; the daemon should close the connection.
