...
//     cmd/borgee main.go (daemon subcommand)
...
// buildDaemon builds the borgee binary into a tempdir and returns its path.
// The daemon is invoked via the `borgee daemon` subcommand (folded from the
// prior cmd/borgee-helper entry point).
...
func TestHB2DE_DaemonStartup_BuildsAndListens(t *testing.T) {
	...
	tmp := t.TempDir()
	socketPath := filepath.Join(tmp, "borgee.sock")
