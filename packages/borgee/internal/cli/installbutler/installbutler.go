...
//	  → chown borgee:borgee when running as root
//	  → exit 0 (or 1 on any of the seven failure modes)
//
// DO NOT turn this into a long-lived daemon. The whole point of the
// `install` subcommand is "one-shot + drop privilege by exiting". If you need
// background scheduling (e.g. nightly update polling), wrap it from a
// systemd timer or launchd `StartCalendarInterval`, NOT from this code path.
// Lingering as root after the write would defeat the install /
...
// Reason strings — `borgee install` structured stderr error codes. Each maps
...
// Kept as a local struct because borgee is a separate Go module and must not
// import server-go internal packages.
...
// Run is the entry for `borgee install`. Dispatcher in cmd/borgee passes the
// remaining argv + stdio. Returns a non-nil error when the subcommand should
// exit 1; the calling main is responsible for translating to the process exit
// code so each subcommand can stay io.Writer-friendly under test.
func Run(args []string, stdout, stderr io.Writer) error {
	return run(args, stdout, stderr)
}

// run is the testable entrypoint. Returns a non-nil error when `borgee
...
func run(args []string, stdout, stderr io.Writer) error {
	...
	helperUser := fs.String("helper-user", "borgee", "OS user to chown the target to when running as root (default borgee)")
	...
	installedVersionsPath := fs.String("installed-versions-path", "/var/lib/borgee/installed-versions.json", "Path to the installed-versions snapshot file `borgee install` updates after successful install (#999 update detection). Set to empty to disable.")
	...
	// 10. Drop privilege by exiting. borgee install is one-shot; there is
...
// can read while only root / borgee install invocations can write.
func recordInstalledVersion(path, pluginID, version, sha256 string, installedAt int64) error {
	...
	if data, err := os.ReadFile(path); err == nil && len(data) > 0 {
		// Tolerate corrupt file by starting fresh — borgee install is the
