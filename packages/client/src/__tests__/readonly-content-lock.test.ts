// readonly-content-lock.test.ts — CHN-15 exact-match 跨 server/client/
// content-lock 双向锁验证.
//
// Pins:
//   - READONLY_BIT === 16 (跟 server api.ReadonlyBit exact-match)
//   - READONLY_LABEL 3 文案 exact-match 跟 content-lock §1 同源
//   - CHANNEL_READONLY_TOAST 1 字面 exact-match 跟 server const
...
describe('CHN-15 exact-match 双向锁', () => {
  it('READONLY_BIT === 16 (跟 server api.ReadonlyBit exact-match)', () => {
    expect(READONLY_BIT).toBe(16);
  });

  it('READONLY_LABEL 3 文案 exact-match 跟 content-lock §1 同源', () => {
  ...
  it('CHANNEL_READONLY_TOAST 1 错码 exact-match 跟 server const + content-lock §3', () => {
