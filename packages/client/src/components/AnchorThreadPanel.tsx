...
// §1.6 (anchor discussions = owner review agent 产物的工具). Spec brief:
...
// docs/qa/cv-2-content-lock.md (7 required UI strings).
//
// 设计反查 (cv-2-spec.md §0):
//   - ① anchor = 人审 agent 产物 — UI 入口仅 owner / human (DOM constraint:
...
//   - ③ AnchorCommentAdded envelope 仅信号 (10 fields kept in sync with
...
interface Props {
   ...
   * stale label can render the exact content-lock text for version drift.
   */
...
// Content-lock constants — exact text matches docs/qa/cv-2-content-lock.md
...
function staleLabel(anchorVersion: number, headVersion: number): string {
  // §3.4 exact text matches #358 acceptance + cv-2-content-lock.md ⑦.
    ...
    >
      ...
      <ul className="anchor-comment-list">
        {comments.map((c) => {
          ...
          // ④ kept aligned with CV-1 ArtifactPanel kindBadge.
      ...
      {isResolved && canResolve && (
        <div className="anchor-thread-actions">
          {/* Negative constraint: resolved → reopen 文案锁; v1 服务端不暴露 reopen 路径,
              UI keeps the disabled placeholder text stable; 服务端 v2 加 reopen
              endpoint 时此按钮启用. */}
