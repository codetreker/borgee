...
//   - DM 视图不显示 workspace tab / topic 输入 / 添加成员按钮
//   - DM 不渲染 channel topic banner (DOM 结构与 channel 视图明确区分)
...
//
// 实施约束:
//   - UI 验证通过浏览器执行 (双 BrowserContext + page.click + DOM 断言)
...
// CHN-2 locked copy (#354, content-lock §1 ①⑤).
...
test.describe('CHN-2.3 DM E2E — constraints ②③④⑤⑥ (#357 §1.2 / #354)', () => {
  test('§4.d DM creation + add-member 403 (DM remains two-person)', async () => {
    ...
    // Reverse-fetch from Bob's side returns the same DM, matching the idempotent
    // createDmChannel contract in chn-2.md §2.1 and CHN-1.
    const dmFromBob = await openDmAs(bob, alice.userId);
    expect(dmFromBob.id, 'DM idempotent across both directions').toBe(dm.id);

    // §4.d: POST /channels/:dmId/members must reject on the server.
    // CHN-2.1 #407 covers the server path; this E2E keeps browser-level coverage.
    ...
    expect(addAttempt.status(), 'expected 403/400 reject for DM add-member').toBeGreaterThanOrEqual(
      400,
    );
  });

  test('§4.e DM view DOM — Canvas tab, topic, and 添加成员 are absent', async ({ browser }) => {
    const serverPort = process.env.E2E_SERVER_PORT ?? '4901';
    const serverURL = `http://127.0.0.1:${serverPort}`;
    const adminCtx = await adminLogin(serverURL);
    ...
    // Sidebar shows the locked 私信 group header from content-lock §1 ①
    // (#354 + Sidebar.tsx:411).
    await expect(
      alicePage.locator(`[data-kind="dm"].online-header`, { hasText: DM_GROUP_LABEL }),
    ).toBeVisible({ timeout: 5_000 });

    // Click into the DM with bob.
    const dmRow = alicePage
      .locator(`[data-kind="dm"]`)
      .filter({ hasText: bob.displayName })
      .first();
    await dmRow.click();
    await expect(alicePage.locator('.channel-view[data-channel-type="dm"]')).toBeVisible();

    // §4.e DOM absence checks:
    //   No Canvas / Workspace / Remote tab. The entire .channel-view-tabs bar is
    //   gated by `!isDm` in ChannelView.tsx:159, so assert absence of both the
    //   bar and each tab button.
    await expect(
      alicePage.locator('.channel-view[data-channel-type="dm"] .channel-view-tabs'),
    ).toHaveCount(0);
    await expect(
      alicePage.locator('.channel-view[data-channel-type="dm"] .channel-view-tab', {
        hasText: 'Canvas',
      }),
    ).toHaveCount(0);
    await expect(
    ...
    //   No topic banner. .channel-topic only renders for non-DM channels with
    //   topic field set (gated by !isDm + headerTopic).
    await expect(
      alicePage.locator('.channel-view[data-channel-type="dm"] .channel-topic'),
    ).toHaveCount(0);
    //   No 添加成员 / 成员管理 / 离开频道 buttons. Header buttons are gated by
    //   `!isDm && channel` in ChannelView.tsx:138.
    await expect(
      alicePage.locator('.channel-view[data-channel-type="dm"] button', { hasText: '成员管理' }),
    ).toHaveCount(0);
    await expect(
      alicePage.locator('.channel-view[data-channel-type="dm"] button', { hasText: '添加成员' }),
    ).toHaveCount(0);
    await expect(
      alicePage.locator('.channel-view[data-channel-type="dm"] button', { hasText: '离开频道' }),
    ).toHaveCount(0);

    await aliceCtx.close();
  });

  test('§1 ⑤ third-person mention placeholder uses locked DM copy', async ({ browser }) => {
    const serverPort = process.env.E2E_SERVER_PORT ?? '4901';
    const serverURL = `http://127.0.0.1:${serverPort}`;
    const adminCtx = await adminLogin(serverURL);
    ...
    // Do not show copy that implies converting a DM to a channel. Blueprint §1.2
    // requires creating a new channel instead.
  ...
  test('§4.b DM upgrade path does not exist (POST /channels/:id/upgrade-to-channel)', async () => {
    const serverPort = process.env.E2E_SERVER_PORT ?? '4901';
    const serverURL = `http://127.0.0.1:${serverPort}`;
    const adminCtx = await adminLogin(serverURL);
    ...
    // endpoint must not exist because adding people requires creating a channel,
    // not converting the DM.
    // Probe gives 404 (route not registered) or 405 (route exists for
    // different method). Both signal "no upgrade path"; success would
    // signal an unsupported upgrade path.
  ...
  test('§3 DM message text node does not expose raw UUID', async ({ browser }) => {
    const serverPort = process.env.E2E_SERVER_PORT ?? '4901';
    const serverURL = `http://127.0.0.1:${serverPort}`;
    const adminCtx = await adminLogin(serverURL);
    ...
    // ID exposure check: match either UUID-36 hex or ULID-26 Crockford base32
    ...
    // data-mention-id may carry Bob's UUID; the visible text node must not.
